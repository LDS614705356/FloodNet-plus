...
class CoTransformerLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        co_att = self.comhatt(att, keys, values, attention_mask, attention_weights) # co attn
        # alpha1 = self.fc(torch.cat(co_att, att), -1)
        # alpha1 = torch.sigmoid(alpha1, dim=0)
        ff = self.pwff(co_att) # FFN
        return ff
...
class GridFeatureNetwork(nn.Module):
    ...
    def forward(self, input, seq_input, attention_mask=None, attention_weights=None):
        ...
        outs = []
        text_outs = []
        for l, cl in zip(self.layers, self.co_layer):
