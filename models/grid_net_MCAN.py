...
class TransformerLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        # att = self.comhatt(att, keys, values, attention_mask, attention_weights)  # co attn
        ff = self.pwff(att)
        return ff
...
class TextTransformerLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        att = self.mhatt(queries, queries, queries, attention_mask, attention_weights) # self attn
        # att = self.comhatt(att, keys, values, attention_mask, attention_weights) # co attn
        ff = self.pwff(att) # FFN
        return ff
...
class GridFeatureNetwork(nn.Module):
    ...
    def forward(self, input, seq_input, attention_mask=None, attention_weights=None):
        ...
        for tl in self.text_layers:
            ...
            # y = tl(y, y, y, None, None)
            # y = cl(y, out, out, None, None)

            # text_outs.append(y.unsqueeze(1))

        for l in self.layers:
            out = l(out, y, y, attention_mask, attention_weights)
            # out = cl(out, y, y, None, None)
        ...
        out = self.add_layer[0](out, out, out, None, None)
        out = self.add_layer[1](out, out, out, None, None)
        outs.append(out.unsqueeze(1))
