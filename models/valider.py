...
class captionTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        # self.grid_fw = define_grid_fw(args=args, gpu_ids=args.gpu_ids)
        # self.junction = define_junction(args=args, gpu_ids=args.gpu_ids)
        self.TransFormer = define_transformer(args=args, gpu_ids=args.gpu_ids)
        # if self.args.net_G == "ccnet":
        #     self.downsample = define_downsample(args=args, gpu_ids=args.gpu_ids)
        ...
        if args.optimizer == "sgd":
            self.optimizer_G = optim.SGD(self.net_G.parameters(), lr=self.lr*0.01,
                                         momentum=0.9,
                                         weight_decay=5e-4)
            # self.optimizer_fw = optim.SGD(self.grid_fw.parameters(), lr=self.lr,
            #                              momentum=0.9,
            #                              weight_decay=5e-4)
            # self.optimizer_j = optim.SGD(self.junction.parameters(), lr=self.lr,
            #                              momentum=0.9,
            #                              weight_decay=5e-4)
            self.optimizer_T = optim.SGD(filter(lambda p: p.requires_grad, self.TransFormer.parameters()), lr=self.lr,
                                         ...
                                         weight_decay=5e-4)
            # if self.args.net_G == "ccnet":
            #     self.optimizer_d = optim.SGD(self.downsample.parameters(), lr=self.lr,
            #                              momentum=0.9,
            #                              weight_decay=5e-4)
            ...
            self.optimizer_G = optim.Adam(self.net_G.parameters(), lr=self.lr*0.01,
                                          weight_decay=0)
            # self.optimizer_fw = optim.Adam(self.grid_fw.parameters(), lr=self.lr,
            #                               weight_decay=0)
            # self.optimizer_j = optim.Adam(self.junction.parameters(), lr=self.lr,
            #                               weight_decay=0)
            self.optimizer_T = optim.Adam(filter(lambda p: p.requires_grad, self.TransFormer.parameters()), lr=self.lr,
                                          weight_decay=0)
            # if self.args.net_G == "ccnet":
            #     self.optimizer_d = optim.Adam(self.downsample.parameters(), lr=self.lr,
            #                              weight_decay=0)

        elif args.optimizer == "adamw":
            self.optimizer_G = optim.AdamW(self.net_G.parameters(), lr=self.lr*0.01,
                                           betas=(0.9, 0.999), weight_decay=0.01)
            # self.optimizer_fw = optim.AdamW(self.grid_fw.parameters(), lr=self.lr,
            #                                betas=(0.9, 0.999), weight_decay=0.01)
            # self.optimizer_j = optim.AdamW(self.junction.parameters(), lr=self.lr,
            #                                betas=(0.9, 0.999), weight_decay=0.01)
            self.optimizer_T = optim.AdamW(filter(lambda p: p.requires_grad, self.TransFormer.parameters()), lr=self.lr,
                                           betas=(0.9, 0.999), weight_decay=0.01)
            # if self.args.net_G == "ccnet":
            #     self.optimizer_d = optim.AdamW(self.downsample.parameters(), lr=self.lr,
            #                              betas=(0.9, 0.999), weight_decay=0.01)
        # self.optimizer_G = optim.Adam(self.net_G.parameters(), lr=self.lr)

        # define lr schedulers
        self.exp_lr_scheduler_G = get_scheduler(self.optimizer_G, args)
        # self.exp_lr_scheduler_fw = get_scheduler(self.optimizer_fw, args)
        # self.exp_lr_scheduler_j = get_scheduler(self.optimizer_j, args)
        ...
        # self.net_G, self.optimizer_G = amp.initialize(self.net_G, self.optimizer_G, opt_level='O1')
        # print(amp.state_dict())

        # self.junction, self.optimizer_j = amp.initialize(self.junction, self.optimizer_j, opt_level='O1')
        # self.grid_fw, self.optimizer_fw = amp.initialize(self.grid_fw, self.optimizer_fw, opt_level='O1')
        # self.TransFormer, self.optimizer_T = amp.initialize(self.TransFormer, self.optimizer_T, opt_level='O1')
        # print(amp.state_dict())
    ...
    def _load_checkpoint(self, ckpt_name='best_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            ...
            # update some other states
            self.epoch_to_start = checkpoint['epoch_id'] + 1
            self.best_val_acc = checkpoint['vqa_accuracy']
            self.logger.write("best acc: " + str(self.best_val_acc) + "\n")
            self.logger.write("best epoch: " + str(checkpoint['epoch_id']) + "\n")
            # self.best_epoch_id = checkpoint['best_epoch_id']
            ...
            # self.junction.to(self.device)
            # self.grid_fw.to(self.device)
            self.TransFormer.to(self.device)
    ...
    def _save_checkpoint(self, ckpt_name):
        torch.save({
            ...
            'model_G_state_dict': self.net_G.state_dict(),
            # 'model_j_state_dict': self.junction.state_dict(),
            # 'model_fw_state_dict': self.grid_fw.state_dict(),
            'model_T_state_dict': self.TransFormer.state_dict(),
            # 'amp': amp.state_dict(),
            'optimizer_G_state_dict': self.optimizer_G.state_dict(),
            # 'optimizer_j_state_dict': self.optimizer_j.state_dict(),
            # 'optimizer_fw_state_dict': self.optimizer_fw.state_dict(),
            'optimizer_T_state_dict': self.optimizer_T.state_dict(),
            'exp_lr_scheduler_G_state_dict': self.exp_lr_scheduler_G.state_dict(),
            # 'exp_lr_scheduler_j_state_dict': self.exp_lr_scheduler_j.state_dict(),
            # 'exp_lr_scheduler_fw_state_dict': self.exp_lr_scheduler_fw.state_dict(),
    ...
    def _update_checkpoints(self,score):
        ...
        # save current model
        self._save_checkpoint(ckpt_name='last_ckpt.pt')
        self.logger.write('Lastest model updated. Epoch_Cider=%.4f, Historical_best_Cider=%.4f (at epoch %d)\n'
    ...
    def caption_backward(self, seq, current_s_batch):
        ...
        if(self.train_mark == "f"):
            ...
            finetune_loss = self.caption_loss(self.cider, res_compare, self.device, self.args.batch_size, self.args.beam_size, self.log_probs, seq_mask, self.best_cider_D, weights)
    ...
    def train_models(self):
        ...
        for self.batch_id, batch in enumerate(self.dataloaders['val'], 0):
            ...
            name = batch["name"]
            for i in range(len(batch_tuple_A)):
        ...
        dirpath = "val_test"+str(self.epoch_id)+".json"
        ...
        self.logger.write(f"inference_time: " + str(self.time_collect/(self.batch_id + 1)) + "\n")
